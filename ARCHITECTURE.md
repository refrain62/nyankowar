...
- **ビルドの不変性:**
    - `Lint` および `Test` ジョブがすべてパスした場合にのみ `Production Build` を実行し、デプロイ品質を 100% 維持。
    - 本番ビルド用の `tsconfig.app.json` において `src/**/*.stories.tsx` を明示的に `exclude` に追加。これにより、ビルド環境に Storybook 固有の型定義が存在しない場合でも、コアロジックのコンパイルを安全に完遂させる。
    - すべてのストーリーファイル内の `play` 関数に型アノテーションを付与し、CI 時の静的解析における `any` 混入を物理的に排除する。
