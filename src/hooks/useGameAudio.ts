...
import { playDefeat, playVictory } from "../audio/music/jingles";
import {
	playCannonCharge,
	playCannonExplosion,
	playGashi,
} from "../audio/se/battle";
...
export const useGameAudio = () => {
	...
	return useMemo(
		() => ({
			...
			playCharinSound: () => {
				if (isAudioEnabledRef.current) playCharin(getCtx());
			},
			playUpgradeSound: () => {
				if (isAudioEnabledRef.current) playUpgrade(getCtx());
			},
			playGashiSound: () => {
				if (isAudioEnabledRef.current) playGashi(getCtx());
			},
			playCannonChargeSound: () => {
				if (isAudioEnabledRef.current) playCannonCharge(getCtx());
			},
			playCannonExplosionSound: () => {
				if (isAudioEnabledRef.current) playCannonExplosion(getCtx());
			},
			playVictoryFanfare: () => {
				if (isAudioEnabledRef.current) playVictory(getCtx());
			},
			playDefeatJingle: () => {
				if (isAudioEnabledRef.current) playDefeat(getCtx());
			},
		}),
		[
			isAudioEnabled,
